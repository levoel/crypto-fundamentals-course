...
<KnowledgeCheck
  question="Как Subsquid processor декодирует Ethereum адрес из log.topics[1]?"
  answer="topics[1] содержит 32 байта (66 hex символов с 0x prefix). Адрес -- последние 20 байт (40 hex). .slice(26) отбрасывает '0x' (2 символа) + zero-padding (24 символа). Результат: '0x' + topics[1].slice(26).toLowerCase() = корректный Ethereum адрес в lowercase."
/>
...
<KnowledgeCheck
  question="Почему в Subsquid Transfer используется batch INSERT, а Account -- individual UPSERT?"
  answer="Transfer -- immutable event (факт, который не меняется после создания). ctx.store.insert(transfers) -- один SQL для всего массива. Account -- mutable state (баланс обновляется при каждом трансфере). ctx.store.upsert(account) -- обновляет существующую запись. Разные паттерны persistence для разных типов данных."
/>
