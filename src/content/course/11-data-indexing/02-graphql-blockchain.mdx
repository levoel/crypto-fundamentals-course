...
<KnowledgeCheck
  question="Что такое @entity и @index в schema.graphql и для чего они нужны?"
  answer="@entity -- тип = таблица в PostgreSQL (аналог CREATE TABLE). @index -- B-tree индекс на поле (аналог CREATE INDEX). @index ускоряет WHERE и ORDER BY с O(N) full scan до O(log N) seek. Пример: from: String! @index создает индекс для быстрого поиска по отправителю."
/>

## Запросы (Queries)

<GraphQLQueryBuilderDiagram client:visible />
...
<KnowledgeCheck
  question="Какие операторы фильтрации доступны в GraphQL для блокчейн-данных?"
  answer="_eq (равно), _gt (больше), _gte (больше или равно), _lt (меньше), _lte (меньше или равно), _in (в списке), _contains (содержит подстроку), _startsWith (начинается с). Пример: transfers(where: { value_gte: '1000', from_eq: '0x...' })."
/>
