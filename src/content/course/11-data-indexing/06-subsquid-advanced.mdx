...
<KnowledgeCheck
  question="Чем stateful индексация (governance) отличается от stateless (ERC-20 Transfer)?"
  answer="Stateless: каждое событие -- самодостаточная запись (INSERT). Transfer произошёл -- записали, запись не меняется. Stateful: entity имеет жизненный цикл. Proposal: Created -> Active -> Succeeded -> Executed. Каждое новое событие (VoteCast, ProposalExecuted) обновляет entity через UPSERT."
/>
...
<KnowledgeCheck
  question="Когда следует использовать WebSocket подписки вместо polling?"
  answer="Для real-time dashboards: governance голосования, live event feed, DeFi price ticker. Subscription автоматически push-ит данные при появлении новых записей через WebSocket. Предупреждение: RAM подписок масштабируется плохо. Для тысяч подключений -- используйте polling или внешний PubSub."
/>
