...
<KnowledgeCheck
  question="Почему прямые RPC-запросы (eth_getLogs) не масштабируются для dashboard?"
  answer="Четыре ограничения: 1) Timeout на больших диапазонах (20M+ блоков). 2) Нет агрегаций (SUM, GROUP BY). 3) Повторные вычисления каждым пользователем. 4) Raw hex данные требуют ручного ABI-декодирования. Индексатор решает все четыре: один ETL-проход, затем O(log N) запросы через PostgreSQL."
/>
...
<KnowledgeCheck
  question="Назовите три основных инструмента для индексации блокчейн-данных и их ключевые различия."
  answer="Subsquid: TypeScript, batch processing (1K-50K блоков/сек), hot blocks. The Graph: AssemblyScript, sequential (~100 блоков/сек), зрелая децентрализованная сеть (GRT). SubQuery: TypeScript, sequential (~300 блоков/сек), нативная multi-chain поддержка."
/>
...
Помимо классической связки RPC + индексатор + GraphQL, TON предлагает свой стек инфраструктурных сервисов: TON DNS для имён, TON Storage для распределённого хранения, TON Proxy/Sites для приватной сети. Эти сервисы дополняют индексаторы и местами заменяют их (например, для отдачи статики децентрализованного фронтенда).

<CrossCourseRef
  courseId="ton-blockchain"
  lessonSlug="09-ton-services/01-ton-dns"
  label="TON Services: DNS, Storage, Proxy"
  preview="Инфраструктурные сервисы TON и их роль в стеке dApp -- альтернативы и дополнения к классическим индексаторам"
/>
