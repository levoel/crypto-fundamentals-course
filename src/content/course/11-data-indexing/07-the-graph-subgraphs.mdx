...
<SubgraphManifestDiagram client:visible />
...
<KnowledgeCheck
  question="Из каких трёх файлов состоит каждый subgraph в The Graph?"
  answer="1. subgraph.yaml -- манифест: ЧТО индексировать (адрес контракта, ABI, startBlock, eventHandlers). 2. schema.graphql -- КАК хранить (entities, идентично Subsquid). 3. src/mapping.ts -- КАК обрабатывать (AssemblyScript маппинги, компилируемые в WASM)."
/>

## AssemblyScript: НЕ TypeScript!

<AssemblyScriptComparisonDiagram client:visible />
...
<KnowledgeCheck
  question="Почему AssemblyScript -- это НЕ TypeScript, несмотря на расширение .ts?"
  answer="AssemblyScript компилируется в WebAssembly, не в JavaScript. Ключевые ограничения: нет async/await (все операции синхронные), нет Array.filter/map/reduce, нет closures, нет JSON.parse, нет try/catch. entity.save() -- синхронный. Используйте for loops вместо функциональных методов."
/>
