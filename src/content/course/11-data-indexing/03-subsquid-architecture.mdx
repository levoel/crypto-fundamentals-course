...
<KnowledgeCheck
  question="Почему batch processing в Subsquid обеспечивает 50-300x преимущество в скорости перед The Graph?"
  answer="Subsquid обрабатывает сотни событий за раз: один batch INSERT вместо сотен отдельных. The Graph обрабатывает последовательно: entity.save() для каждого события = отдельный SQL запрос. Каждый SQL имеет overhead (parsing, planning, network). Batch сокращает overhead в N раз."
/>
...
<KnowledgeCheck
  question="Какие 5 шагов составляют конвейер кодогенерации Subsquid?"
  answer="1. squid-typeorm-codegen (schema.graphql -> TypeORM entities). 2. squid-evm-typegen (ABI JSON -> typed decoders). 3. tsc (TypeScript -> JavaScript). 4. squid-typeorm-migration generate (entities -> SQL migrations). 5. squid-typeorm-migration apply (migrations -> PostgreSQL tables)."
/>
