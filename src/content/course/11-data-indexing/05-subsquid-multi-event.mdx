...
<KnowledgeCheck
  question="Как один EvmBatchProcessor обрабатывает несколько типов событий?"
  answer="Несколько .addLog() фильтров (Transfer, Swap, Sync). Все совпадающие логи попадают в один батч. В handler -- switch по log.topics[0] для маршрутизации: TRANSFER_TOPIC -> decodeTransfer(), SWAP_TOPIC -> decodeSwap(), SYNC_TOPIC -> updatePoolState()."
/>
...
<KnowledgeCheck
  question="Почему PoolState обновляется через UPSERT, а не batch INSERT?"
  answer="PoolState -- mutable state (один пул). Каждое Sync событие перезаписывает reserve0 и reserve1 той же записи. Если в одном батче два Sync события, нужно применить оба последовательно. Batch INSERT создает новые записи, а не обновляет существующие."
/>
