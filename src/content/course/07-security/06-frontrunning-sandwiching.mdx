...
<KnowledgeCheck
  question="Какой параметр в Uniswap Router определяет защиту от sandwich attack?"
  answer="amountOutMin -- минимально допустимый output. Если output ниже этого значения, транзакция ревертится. amountOutMin = 0 -- приглашение к sandwich: searcher может сдвинуть цену на любую величину. Рекомендация: 95-99% от expected output."
/>
...
<MempoolVisualizationDiagram client:visible />
...
<KnowledgeCheck
  question="Почему публичный mempool делает frontrunning возможным?"
  answer="Все pending транзакции видны всем узлам. Searcher мониторит mempool, декодирует calldata (swap parameters, amountOutMin), симулирует profit и создает sandwich bundle. Решение: приватный mempool (Flashbots Protect) -- tx не появляется в публичном mempool."
/>
