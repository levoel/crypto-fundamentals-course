...
<KnowledgeCheck
  question="Какие три уровня защиты предоставляет Flashbots Protect?"
  answer="1) Sandwich protection -- tx не в публичном mempool, searchers не видят её. 2) Failed tx protection -- если tx ревертится, builder не включает в блок, вы не платите gas. 3) MEV refund -- 90% MEV от backrun возвращается пользователю."
/>
...
<KnowledgeCheck
  question="Почему MEV-Share перераспределяет часть MEV обратно пользователям?"
  answer="Рыночная конкуренция: builders, предлагающие больший refund, получают больше order flow (пользовательских tx). Builder, не возвращающий MEV, теряет пользователей. Flashbots Protect реализует это: 90% прибыли от backrun отправляется обратно пользователю в том же блоке."
/>
