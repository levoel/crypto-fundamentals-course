...
<SlitherOutputGuideDiagram client:visible />
...
<KnowledgeCheck
  question="Как Slither анализирует контракт -- выполняет код или разбирает структуру?"
  answer="Slither -- статический анализатор: разбирает AST (Abstract Syntax Tree) Solidity-кода без выполнения. Pipeline: Solidity -> Compile -> AST -> SlithIR -> Detectors -> Findings. Анализирует control flow, data flow, dependencies. 200+ детекторов, запуск за секунды."
/>
...
<KnowledgeCheck
  question="Почему layered defense (Slither + Mythril + Manual) находит больше уязвимостей, чем любой метод по отдельности?"
  answer="Каждый метод имеет разные strengths: Slither ловит known patterns за секунды, Mythril через symbolic execution находит conditional/multi-tx bugs, manual review обнаруживает business logic и economic attacks. Пересечение их capabilities минимально, комбинация дает ~95%+ coverage."
/>
...
|---------|---------|---------|--------|
| Reentrancy in withdraw() | [OK] (reentrancy-eth) | [OK] (SWC-107) | TP |
| Unprotected initialize() | [OK] (unprotected-upgrade) | ? | TP |
| Event after call | [OK] (reentrancy-events) | [NO] | Info |
| ...     | ...     | ...     | ...    |
```
