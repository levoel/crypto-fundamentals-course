...
<KnowledgeCheck
  question="Какой процент уязвимостей находят automated tools, а какой -- manual review?"
  answer="Automated tools (Slither, Mythril) находят ~20% уязвимостей (known patterns, SWC). Manual review находит ~80% (business logic, economic attacks, cross-contract). Вместе = ~95%+ coverage. Аудит БЕЗ manual review = security theater."
/>
...
<KnowledgeCheck
  question="Какой severity присваивается уязвимости, позволяющей вывести ВСЕ средства из контракта?"
  answer="Critical -- потеря ВСЕХ средств, необратимо. Примеры: reentrancy drain, unprotected selfdestruct. Действие: немедленный fix, блокировка деплоя. Каждый finding должен включать PoC -- без PoC finding = теория, не доказательство."
/>
