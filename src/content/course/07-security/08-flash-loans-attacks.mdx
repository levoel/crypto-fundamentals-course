...
<KnowledgeCheck
  question="Flash loan -- это уязвимость или инструмент? Что является настоящей уязвимостью в flash loan атаках?"
  answer="Flash loan -- инструмент (как молоток). Настоящая уязвимость -- использование spot price (getReserves) как oracle. Spot price манипулируем в 1 транзакции. Flash loan просто предоставляет капитал для манипуляции без залога."
/>
...
<DeFiAttacksTimelineDiagram client:visible />
...
<KnowledgeCheck
  question="Почему Chainlink oracle устойчив к flash loan манипуляции, а getReserves() -- нет?"
  answer="getReserves() отражает текущие резервы DEX, которые можно сдвинуть на 80%+ одним свопом (стоимость = $0 с flash loan). Chainlink агрегирует данные от 20+ независимых sources -- манипуляция требует контроля над большинством узлов (стоимость > $1B)."
/>
