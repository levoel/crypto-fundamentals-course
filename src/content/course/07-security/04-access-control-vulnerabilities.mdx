...
<KnowledgeCheck
  question="В UnsafeToken функция mint() объявлена как public без модификатора. Какие атаки это позволяет?"
  answer="Бесконечный mint: любой адрес минтит произвольное количество токенов и продает на DEX, обрушая цену для всех держателей. Если burn(from, amount) тоже без AC, атакующий сжигает токены других пользователей. Одна пропущенная строка onlyOwner = катастрофа."
/>
...
```mermaid
graph TD
    A["DEFAULT_ADMIN_ROLE<br/>(может назначать любые роли)"] --> B["MINTER_ROLE<br/>(может минтить)"]
    A --> C["PAUSER_ROLE<br/>(может приостанавливать)"]
    A --> D["UPGRADER_ROLE<br/>(может обновлять proxy)"]
    A --> E["CUSTOM_ROLE<br/>(кастомная роль)"]
```
...
<KnowledgeCheck
  question="Чем AccessControl (RBAC) лучше Ownable для DeFi-протоколов?"
  answer="RBAC назначает отдельные роли (MINTER, PAUSER, UPGRADER) разным адресам. Компрометация одного ключа не дает полный контроль. С Ownable все функции контролирует один адрес -- single point of failure. RBAC + multi-sig + timelock = defense in depth."
/>
