...
<KnowledgeCheck
  question="Какие 6 полей содержит заголовок блока Bitcoin и каков его размер?"
  answer="Заголовок блока -- ровно 80 байт: version (4), Previous Block Hash (32), Merkle Root (32), timestamp (4), nBits (4, компактное представление target), nonce (4). Именно эти 80 байт хешируются через SHA-256d для получения block hash."
/>
...
<KnowledgeCheck
  question="Почему изменение одной транзакции в блоке N делает невалидными ВСЕ последующие блоки?"
  answer="Изменение транзакции меняет Merkle Root блока N, что меняет хеш блока N. Поле Previous Hash в блоке N+1 больше не совпадает, нужно пересчитать PoW для N+1 (найти новый nonce), затем для N+2, N+3 и т.д. Для обгона честной цепочки нужно >50% хешрейта сети."
/>
...
<KnowledgeCheck
  question="Зачем нужен Merkle Root в заголовке блока? Какие преимущества он даёт?"
  answer="Merkle Root (32 байта) фиксирует любое количество транзакций блока. Преимущества: компактность (32 байта для тысяч транзакций), возможность Merkle Proof для SPV-узлов (проверка включения транзакции за O(log n)), неизменяемость (любое изменение транзакции меняет Merkle Root и хеш блока)."
/>
