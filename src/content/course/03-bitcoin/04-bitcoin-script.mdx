...
<KnowledgeCheck
  question="Как Bitcoin Script проверяет транзакцию? Какие два скрипта выполняются и в каком порядке?"
  answer="Сначала выполняется scriptSig (скрипт-ключ из входа транзакции), его результат остаётся на стеке. Затем на том же стеке выполняется scriptPubKey (скрипт-замок из UTXO). Если после выполнения обоих скриптов на стеке одно ненулевое значение (true) -- транзакция валидна."
/>
...
<P2PKHStackAnimation client:visible />
...
<OpcodeReferenceDiagram client:visible />
...
<KnowledgeCheck
  question="Какие 5 шагов выполняются при проверке P2PKH-скрипта (Pay-to-Public-Key-Hash)?"
  answer="1) Push <sig> и <pubKey> на стек (из scriptSig). 2) OP_DUP -- дублирует pubKey. 3) OP_HASH160 -- хеширует копию (SHA-256 + RIPEMD-160). 4) OP_EQUALVERIFY -- сравнивает хеш с pubKeyHash из scriptPubKey. 5) OP_CHECKSIG -- проверяет ECDSA-подпись. Если подпись валидна -- true на стеке."
/>
...
<KnowledgeCheck
  question="Почему Turing-неполнота Bitcoin Script -- это преимущество, а не недостаток?"
  answer="Каждый узел сети выполняет каждый скрипт каждой транзакции. Без циклов гарантируется завершение (нет бесконечных скриптов), без состояния -- изоляция, ограниченный стек (1000 элементов) -- предсказуемые ресурсы. Ethereum решает это через gas, а Bitcoin -- ограничением самого языка, что фундаментально безопаснее."
/>
