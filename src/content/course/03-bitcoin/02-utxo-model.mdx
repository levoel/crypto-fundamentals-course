...
<KnowledgeCheck
  question="Почему в Bitcoin нет понятия 'баланс'? Как кошелёк вычисляет доступные средства?"
  answer="В Bitcoin нет поля 'balance' в базе данных. Кошелёк отслеживает набор UTXO (Unspent Transaction Outputs) -- неизрасходованных выходов, которые он может потратить. Баланс -- это сумма amount всех доступных UTXO. Каждый UTXO -- это 'купюра' определённого номинала."
/>
...
<KnowledgeCheck
  question="Как вычисляется комиссия (fee) в Bitcoin-транзакции? Что произойдёт, если забыть создать выход для сдачи?"
  answer="fee = sum(inputs) - sum(outputs). В транзакции нет явного поля 'fee' -- комиссия это просто разница между входами и выходами. Если забыть создать выход для сдачи, ВСЯ разница уйдёт майнеру как комиссия. Например, при входе 1 BTC и выходе 0.3 BTC без сдачи -- 0.7 BTC станут комиссией."
/>
...
<UTXOSetVisualization client:visible />
...
<ChangeAddressDiagram client:visible />
...
<KnowledgeCheck
  question="Почему сдача в Bitcoin отправляется на новый адрес, а не на исходный?"
  answer="HD-кошельки (BIP 32/44/84) генерируют новый адрес для каждой операции сдачи. Это затрудняет анализ цепочки транзакций (chain analysis): наблюдатель не может определить, какой из выходов -- оплата, а какой -- сдача, что повышает приватность пользователя."
/>
