...
<KnowledgeCheck
  question="Какие три роли есть в TimelockController и почему renounce DEFAULT_ADMIN_ROLE критически важен?"
  answer="PROPOSER_ROLE (Governor -- ставит операции в очередь), EXECUTOR_ROLE (address(0) = anyone -- исполняет после delay), DEFAULT_ADMIN_ROLE (deployer -- управление ролями). Если deployer не renounce admin, он может обойти governance: grantRole(PROPOSER, self) и исполнить любое действие без голосования."
/>
...
<KnowledgeCheck
  question="Что такое exit opportunity и почему timelock delay дает пользователям реальную возможность защитить свои средства?"
  answer="Exit opportunity -- ключевая концепция: все pending actions в timelock прозрачны on-chain. Пользователи видят, что произойдет после исполнения (targets, values, calldatas). Если они не согласны с одобренным предложением, у них есть время (delay >= 1 день) вывести средства из протокола ДО исполнения."
/>
