...
<KnowledgeCheck
  question="Из каких 3 контрактов состоит полная DAO реализация на OpenZeppelin Governor и какова роль каждого?"
  answer="GovernanceToken (ERC20Votes: voting power, checkpoints, delegation) + MyGovernor (proposal lifecycle, голосование, 4 extensions) + TimelockController (задержка исполнения, 3 роли). Дополнительно Treasury (onlyOwner = timelock, хранение средств). _executor() возвращает адрес timelock -- timelock является фактическим владельцем контролируемых контрактов."
/>
...
<KnowledgeCheck
  question="Почему в Foundry тесте полного governance lifecycle используется vm.warp() и почему каждый voter должен вызвать delegate() перед голосованием?"
  answer="vm.warp() сдвигает block.timestamp для прохождения votingDelay (1 day), votingPeriod (1 week), и timelockDelay (1 day) -- иначе тест длился бы 9 реальных дней. delegate() обязателен: без него getVotes(voter) = 0 даже при balanceOf > 0 -- голос не будет засчитан. ERC20Votes не создает checkpoints автоматически."
/>
