...
<KnowledgeCheck
  question="Каковы три шага Schnorr identification protocol?"
  answer="1. Commit: Prover выбирает случайное k, вычисляет R = kG и отправляет Verifier. 2. Challenge: Verifier выбирает случайный e. 3. Response: Prover вычисляет s = k + e*x mod n. Верификация: s*G == R + e*P."
/>
...
<KnowledgeCheck
  question="Как Fiat-Shamir transform превращает interactive proof в non-interactive?"
  answer="Заменяет случайный challenge от Verifier на хеш от публичных данных: c = H(statement || R). Prover не может предсказать c до вычисления R, потому что изменение R непредсказуемо меняет c. Хеш-функция 'имитирует' честного Verifier."
/>
