...
<KnowledgeCheck
  question="Какие два свойства ОБЯЗАН обеспечивать любой commitment scheme?"
  answer="Binding (привязка): после commit нельзя изменить значение. Hiding (сокрытие): до reveal нельзя определить запечатанное значение. Невозможно иметь ОБА свойства information-theoretic одновременно."
/>
...
<HashVsPedersenDiagram client:visible />
...
<HomomorphicCommitmentDiagram client:visible />
...
<KnowledgeCheck
  question="В чём ключевое преимущество Pedersen commitment перед hash-based commitment?"
  answer="Аддитивная гомоморфность: C(a,r1) + C(b,r2) = C(a+b, r1+r2). Можно складывать зашифрованные значения без расшифровки. Используется в Confidential Transactions (Monero, Liquid) для проверки баланса без раскрытия сумм."
/>
