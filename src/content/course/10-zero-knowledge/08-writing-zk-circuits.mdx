...
<KnowledgeCheck
  question="Назовите 8 шагов полного Circom/snarkjs pipeline от .circom файла до verified proof."
  answer="1. Write .circom, 2. Compile (R1CS + WASM), 3. Download Powers of Tau, 4. Groth16 Setup (Phase 2), 5. Contribute Randomness, 6. Export Verification Key, 7. Generate Proof (witness + Groth16), 8. Verify. В Docker lab автоматизировано 3 скриптами: setup.sh, prove.sh, verify.sh."
/>

## Прогрессия circuits: от простого к сложному

<CircuitComplexityDiagram client:visible />
...
<KnowledgeCheck
  question="В чём разница между 'use ZK' и 'build ZK' подходами для разработчиков?"
  answer="Use ZK (90%): деплой на ZK rollups (zkSync, StarkNet), интеграция ZK identity, готовые circuit templates. Не нужно писать circuits. Build ZK (10%): custom privacy/compliance circuits на Circom/Noir/Cairo для уникальных use cases."
/>
