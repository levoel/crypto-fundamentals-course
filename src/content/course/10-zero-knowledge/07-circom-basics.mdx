...
<KnowledgeCheck
  question="Какие три типа signals существуют в Circom и в чём их ключевое свойство?"
  answer="input (private -- Prover знает, Verifier нет), output (public -- Verifier видит), intermediate (внутренний). Ключевое свойство: signals immutable -- после присвоения значение нельзя изменить. Все signals -- элементы конечного поля (числа mod p)."
/>
...
<ConstraintOperatorsDiagram client:visible />
...
<KnowledgeCheck
  question="В чём опасность оператора <-- без парного === в Circom?"
  answer="<-- ТОЛЬКО присваивает значение БЕЗ constraint. Verifier НЕ ПРОВЕРЯЕТ корректность. Malicious Prover может подставить c = 999 вместо a*b = 33, и proof будет считаться валидным. Правило: каждый <-- ОБЯЗАН иметь парный ===."
/>
