...
<KnowledgeCheck
  question="Из чего состоит Groth16 proof и какой его размер?"
  answer="Proof pi = [A, B, C]: A -- элемент G1 (~32 bytes), B -- элемент G2 (~64 bytes), C -- элемент G1 (~32 bytes). Итого ~128 bytes. Размер ПОСТОЯНЕН -- не зависит от сложности доказываемого вычисления."
/>
...
<TrustedSetupDiagram client:visible />
...
<KnowledgeCheck
  question="Что такое 'toxic waste' и почему его уничтожение критически важно?"
  answer="Toxic waste -- секретное значение tau (и промежуточные si участников MPC ceremony). Если кто-то знает tau, он может создавать поддельные proofs для ложных утверждений, фальсифицировать транзакции. MPC ceremony гарантирует: если хотя бы один участник уничтожил свой si, tau невосстановим."
/>
