...
<ZKPropertiesDiagram client:visible />
...
<KnowledgeCheck
  question="Назовите три свойства, которыми ОБЯЗАН обладать любой ZK-протокол."
  answer="Completeness (честный Prover всегда убедит Verifier), Soundness (нечестный Prover не сможет обмануть), Zero-Knowledge (Verifier не узнает ничего, кроме истинности утверждения)."
/>
...
<ZKApplicationsDiagram client:visible />
...
<KnowledgeCheck
  question="Почему non-interactive proofs (SNARKs, STARKs) предпочтительнее interactive proofs для блокчейна?"
  answer="В блокчейне нет 'живого' Verifier -- доказательство публикуется один раз, и тысячи нод верифицируют независимо. Non-interactive proof генерируется Prover однократно, любая нода может проверить позже. Мост между interactive и non-interactive -- Fiat-Shamir transform."
/>
...
В TON commitment schemes и Merkle-доказательства встроены прямо в модель данных через cells: Pruned Branch заменяет поддерево хешем, Merkle Proof доказывает включение, Merkle Update -- изменение состояния. Это пример того, как commitments и proofs становятся первоклассными примитивами на уровне VM.

<CrossCourseRef
  courseId="ton-blockchain"
  lessonSlug="01-ton-architecture/04-cells-boc"
  label="TON cells и Merkle proofs"
  preview="Pruned Branch, Merkle Proof и Merkle Update как первоклассные примитивы TVM"
/>
