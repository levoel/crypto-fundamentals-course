...
<PaymentChannelDiagram client:visible />

<KnowledgeCheck
  question="Сколько on-chain транзакций нужно для state channel и какие 3 этапа составляют его lifecycle?"
  answer="Только 2 on-chain TX: 1) Open -- депозит средств в multisig-контракт. 2) Transact off-chain -- неограниченное число подписанных обновлений (мгновенно, бесплатно, приватно). 3) Close -- публикация финального состояния. Cooperative close = 2 TX независимо от числа off-chain взаимодействий."
/>
...
<KnowledgeCheck
  question="Как dispute resolution в state channel защищает от попытки закрыть канал со старым состоянием?"
  answer="Контракт принимает состояние с наибольшим nonce (самое новое). Если Alice публикует старое состояние (nonce=1), Bob может в течение challenge period опубликовать более новое (nonce=2). Liveness requirement: участник ДОЛЖЕН быть онлайн или иметь watchtower для мониторинга."
/>
