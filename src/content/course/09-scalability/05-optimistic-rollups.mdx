...
<FraudProofComparisonDiagram client:visible />
...
<KnowledgeCheck
  question="Чем отличаются single-round (Cannon) и multi-round (bisection) подходы к fraud proofs?"
  answer="Cannon (Optimism): повторно исполняет спорную транзакцию целиком на L1 через FPVM. Проще, но O(N) gas для больших traces. Bisection (Arbitrum): делит execution trace пополам за log2(N) раундов, пока не найдет одну ошибочную инструкцию. Для 10^8 инструкций: ~27 раундов вместо повторного исполнения всех."
/>
...
<KnowledgeCheck
  question="Почему для безопасности optimistic rollup достаточно одного честного верификатора (1-of-N honest assumption)?"
  answer="Один честный верификатор может загрузить batch data с L1, повторно исполнить транзакции, обнаружить расхождение и подать fraud proof. Для N независимых верификаторов Pr[все коррумпированы] = p^N. При p=0.1 и N=10: Pr[failure] = 10^-10 (99.99999999% безопасность)."
/>
...
<L1L2MessageDiagram client:visible />
