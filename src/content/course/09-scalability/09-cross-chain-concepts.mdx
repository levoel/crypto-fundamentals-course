...
<KnowledgeCheck
  question="Какие 3 модели доверия существуют для cross-chain верификации и как они отличаются по безопасности?"
  answer="1) Natively verified (IBC, rollup bridges): light client/state proof на destination chain, trustless, самый безопасный. 2) Externally verified (Wormhole 13/19): валидаторы подписывают attestation, trust: honest majority. 3) Optimistic (Across): сообщения валидны пока не оспорены, trust: 1 честный watcher. Безопасность убывает от 1 к 3."
/>
...
<KnowledgeCheck
  question="Какие 3 модели передачи активов между блокчейнами существуют и каков risk profile каждой?"
  answer="Lock-and-Mint: lock на source, mint wrapped на destination. Высокий риск (bridge = honeypot). Burn-and-Mint: burn на source, mint native на destination (USDC CCTP). Низкий риск (нет locked assets), но требует поддержки эмитента. Liquidity Pool: deposit/withdraw через LP. Средний риск, быстрый, ограничен ликвидностью."
/>
