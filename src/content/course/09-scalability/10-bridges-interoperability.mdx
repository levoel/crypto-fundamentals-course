...
<BridgeHacksTimelineDiagram client:visible />
...
<KnowledgeCheck
  question="Какой общий паттерн уязвимости объединяет крупнейшие bridge hacks (Wormhole, BNB, Nomad)?"
  answer="Bypass верификации подписей/proofs: Wormhole ($326M) -- signature verification bypass на Solana. BNB ($570M) -- proof verifier bug (подделка IAVL proof). Nomad ($190M) -- initialization bug (trusted root = 0x00, любое сообщение проходит). Proof/signature verification -- критический код: один баг = неограниченный mint."
/>
...
<KnowledgeCheck
  question="Почему Ronin Bridge ($624M) и Harmony Horizon ($100M) были взломаны, и какой урок о threshold multisig?"
  answer="Ronin: 5/9 validator key compromise (Lazarus Group), 4 валидатора от одной организации -- недостаточная децентрализация. Harmony: 2/5 multisig -- слишком низкий threshold. Урок: threshold matters -- 2/5 крайне уязвим (Pr[attack] = 16% при p=0.3), 13/19 (Wormhole) значительно безопаснее (Pr < 10^-12 при p=0.05)."
/>

## Протоколы интероперабельности

<InteropComparisonDiagram client:visible />
