...
<KnowledgeCheck
  question="Какие 4 свойства определяют настоящий Layer 2 и чем L2 отличается от сайдчейна?"
  answer="L2: 1) off-chain execution, 2) публикация данных на L1 (data availability), 3) наследование безопасности L1, 4) exit guarantee (вывод средств на L1 даже если L2 shut down). Сайдчейн имеет собственных валидаторов и собственную безопасность -- если 2/3+ скомпрометированы, Ethereum не может помочь."
/>
...
<DASpectrumDiagram client:visible />
...
<KnowledgeCheck
  question="Почему data availability на L1 -- ключевой различитель между rollup и sidechain?"
  answer="Rollup публикует ВСЕ данные транзакций на L1 (calldata/blobs). Если rollup operator исчезнет, любой может восстановить state из L1 данных. Sidechain хранит данные на собственной цепочке -- если валидаторы скомпрометированы, данные и средства могут быть потеряны. Pr[rollup compromised] <= Pr[L1 compromised] + Pr[L2 bug]."
/>
