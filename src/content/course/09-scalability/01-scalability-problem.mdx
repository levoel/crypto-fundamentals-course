...
<TPSComparisonDiagram client:visible />
...
<KnowledgeCheck
  question="Почему увеличение размера блока не является бесплатным решением проблемы масштабируемости?"
  answer="Больше блоки = больше данных для обработки = дороже оборудование для валидаторов = меньше валидаторов = централизация. Bitcoin Block Size War (2017) привел к хардфорку BTC/BCH. Трилемма: увеличение scalability (T) при сохранении security (S) и decentralization (D) provably hard."
/>

## Ethereum Scaling Roadmap

<ScalingTimelineDiagram client:visible />
...
<KnowledgeCheck
  question="Что означает rollup-centric roadmap Ethereum и какова роль EIP-4844 blobs?"
  answer="Ethereum L1 становится settlement + data availability layer, а execution переносится на L2 rollups. L1 оптимизируется не для TPS, а для security и DA. EIP-4844 ввел blob transactions (~1 gas/byte vs calldata ~16 gas/byte = 10-100x дешевле), хранящиеся ~18 дней -- достаточно для 7-day challenge period optimistic rollups."
/>
...
Альтернативный подход к трилемме -- параллельное исполнение на L1 через actor-model. TON масштабируется не вынесением execution в L2, а тем, что каждый контракт изначально проектируется как независимый актор с асинхронными сообщениями. Глубокий разбор этой парадигмы -- в системном дизайне TON.

<CrossCourseRef
  courseId="sd-ton-developer"
  lessonSlug="03-actor-model/01-actor-model-fundamentals"
  label="System Design TON: actor model"
  preview="Actor-model как способ масштабирования L1: независимые контракты, асинхронные сообщения, параллельное исполнение между шардами"
/>
