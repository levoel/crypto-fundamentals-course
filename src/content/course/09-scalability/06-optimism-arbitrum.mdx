...
```mermaid
graph TD
    ETH["Ethereum L1<br/>(shared security + DA)"] --> OP["Optimism<br/>OP Mainnet<br/>~$2.5B TVL"]
    ETH --> BASE["Base (Coinbase)<br/>~$12B TVL"]
    ETH --> ZORA["Zora<br/>NFT-focused"]
    ZORA --> MODE["Mode<br/>DeFi-focused"]
```
...
```mermaid
graph TD
    ARB["Arbitrum One Network"] --> EVM["EVM<br/>(Solidity, Vyper)"]
    ARB --> WASM["WASM VM<br/>(Stylus)"]
    EVM --> E1["Standard contract execution"]
    WASM --> R["Rust contracts"]
    WASM --> C["C contracts"]
    WASM --> CPP["C++ contracts"]
    EVM <-.->|"Interop"| WASM
```

> Оба VM работают на одной сети! Stylus контракты вызывают Solidity контракты и наоборот.
