...
<KnowledgeCheck
  question="Что такое паттерн Checks-Effects-Interactions (CEI) и почему он критичен?"
  answer="CEI: 1) Checks -- проверить условия (require/revert), 2) Effects -- обновить state (balances), 3) Interactions -- внешние вызовы (call). Порядок критичен: если внешний вызов до обновления state, получатель может повторно вызвать функцию (reentrancy) и обойти проверку баланса. Атака на The DAO (2016): 3.6 млн ETH."
/>
...
<TestWorkflowDiagram client:visible />
...
<KnowledgeCheck
  question="Что делает modifier onlyOwner и как работает символ _; в теле модификатора?"
  answer="onlyOwner проверяет if (msg.sender != owner) revert NotOwner() перед выполнением функции. Символ _; обозначает место, куда подставляется тело функции. Если проверка не пройдена, транзакция откатывается до выполнения логики."
/>
