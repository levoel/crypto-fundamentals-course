...
<NodeArchitectureDiagram client:visible />
...
<KnowledgeCheck
  question="Из каких двух уровней состоит узел Ethereum после The Merge и как они связаны?"
  answer="Execution Layer (EL) -- исполнение транзакций, EVM, state trie. Consensus Layer (CL) -- Beacon Chain, валидаторы, финализация. Связаны через Engine API (JSON-RPC: engine_newPayloadV3, engine_forkchoiceUpdatedV3, engine_getPayloadV3)."
/>
...
<KnowledgeCheck
  question="Почему The Merge считается одним из самых значимых обновлений Ethereum?"
  answer="The Merge заменил энергоёмкий Proof of Work на Proof of Stake, снизив энергопотребление Ethereum на 99.95%. Вместо вычислительной работы (ASIC, электричество) безопасность обеспечивается экономическим залогом (32+ ETH)."
/>
