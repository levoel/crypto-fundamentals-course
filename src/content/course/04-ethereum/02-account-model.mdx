...
<KnowledgeCheck
  question="Какие два типа аккаунтов существуют в Ethereum и чем они управляются?"
  answer="EOA (Externally Owned Account) -- управляется приватным ключом ECDSA, может инициировать транзакции. Contract Account -- управляется байткодом EVM, активируется только транзакцией от EOA или вызовом другого контракта."
/>
...
<KnowledgeCheck
  question="Как вычисляется адрес EOA из публичного ключа?"
  answer="Адрес EOA = последние 20 байт keccak256(uncompressed_public_key). Без 0x04 префикса публичного ключа. Результат: 20 байт = 40 hex символов с 0x-префиксом. В отличие от Bitcoin, Ethereum не использует RIPEMD-160 и Base58."
/>
...
TON применяет принципиально иную модель: каждый аккаунт -- независимый актор со своим деревом ячеек, а вызовы между контрактами всегда асинхронные. Подробное сравнение модели аккаунтов и actor-model TVM -- в курсе по TON.

<CrossCourseRef
  courseId="ton-blockchain"
  lessonSlug="02-tvm-execution/01-actor-model"
  label="TVM: actor model"
  preview="Каждый контракт TON -- независимый актор с собственным состоянием и почтовым ящиком сообщений"
/>
