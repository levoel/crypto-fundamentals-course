...
<KnowledgeCheck
  question="Какие три типа узлов используются в Modified Merkle Patricia Trie?"
  answer="Branch Node (17 элементов: 16 nibble-слотов + value), Extension Node (сжатый общий префикс + указатель на дочерний узел) и Leaf Node (оставшийся путь + RLP-кодированные данные аккаунта)."
/>
...
<KnowledgeCheck
  question="Почему state root в заголовке блока однозначно определяет состояние всей сети?"
  answer="State root -- корневой хеш MPT, вычисленный рекурсивно от всех leaf-узлов (аккаунтов). Изменение одного бита в одном аккаунте перехешивает цепочку от leaf до root, давая другой 32-байтный хеш. Вероятность коллизии: 2^{-256}."
/>
