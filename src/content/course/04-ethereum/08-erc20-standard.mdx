...
<KnowledgeCheck
  question="Какие 6 функций и 2 события определяет стандарт ERC-20?"
  answer="Функции: totalSupply(), balanceOf(address), transfer(to, amount), approve(spender, amount), transferFrom(from, to, amount), allowance(owner, spender). События: Transfer(from, to, value) и Approval(owner, spender, value). Все ERC-20 токены реализуют этот интерфейс."
/>
...
<KnowledgeCheck
  question="Почему ERC-20 использует двухшаговый паттерн approve + transferFrom вместо одного transfer?"
  answer="DEX-контракт не может вызвать transfer() от имени пользователя -- msg.sender будет адресом DEX. Поэтому: 1) пользователь вызывает approve(DEX, amount), устанавливая allowance, 2) DEX вызывает transferFrom(user, recipient, amount), расходуя allowance. ERC-2612 Permit -- off-chain альтернатива."
/>
