...
<KnowledgeCheck
  question="Какие 5 компонентов составляют архитектуру ERC-4337?"
  answer="1) UserOperation -- описание желаемого действия (не транзакция). 2) Bundler -- off-chain сервис, упаковывающий UserOps. 3) EntryPoint -- синглтон-контракт для обработки операций. 4) Smart Account -- контракт с произвольной логикой аутентификации. 5) Paymaster -- контракт для спонсирования газа."
/>

## EIP-7702: легковесная альтернатива

EIP-7702 (часть Pectra, mainnet с 7 мая 2025) -- дополнение к ERC-4337, а не замена. После активации wallet-инфраструктура (Coinbase Smart Wallet, MetaMask Smart Account, Safe) уже добавила поддержку 7702-делегации, и для consumer-кошельков это становится дефолтным способом получить smart-account возможности без миграции адреса:
...
<KnowledgeCheck
  question="Как Paymaster решает проблему cold start для новых пользователей?"
  answer="Paymaster -- смарт-контракт, оплачивающий газ за пользователя. Сценарии: 1) dApp спонсирует все транзакции (пользователю НЕ НУЖЕН ETH). 2) Paymaster принимает ERC-20 (USDC) вместо ETH. 3) Подписочная модель с фиксированной платой. Это устраняет барьер 'сначала купи ETH'."
/>
