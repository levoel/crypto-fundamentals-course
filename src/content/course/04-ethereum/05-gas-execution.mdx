...
<KnowledgeCheck
  question="Как EIP-1559 изменил модель комиссий по сравнению с аукционом первой цены?"
  answer="EIP-1559 ввёл baseFee (автоподстройка под нагрузку, сжигается) + priorityFee (tip валидатору). BaseFee растёт при загрузке >50% целевого размера блока, падает при <50%. Макс. изменение: 12.5% за блок. Это устраняет непредсказуемость и переплаты аукциона первой цены."
/>
...
<KnowledgeCheck
  question="Почему сжигание base fee создаёт дефляционное давление на ETH?"
  answer="Base fee каждой транзакции навсегда удаляется из обращения (burned). При высокой активности сети объём сжигаемого ETH может превышать эмиссию через награды валидаторам, делая ETH дефляционным. При baseFee ~30 gwei сеть сжигает больше ETH, чем создаёт."
/>
