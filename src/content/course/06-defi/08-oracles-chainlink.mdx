...
<KnowledgeCheck
  question="Почему смарт-контракты не могут напрямую вызвать HTTP API для получения цены актива?"
  answer="EVM исполняется детерминированно: все 10,000+ узлов должны получить одинаковый результат. HTTP-ответы недетерминированы (разное время, разные данные, API может упасть). Это сделало бы консенсус невозможным."
/>
...
<KnowledgeCheck
  question="Как Chainlink DON (Decentralized Oracle Network) обеспечивает устойчивость к манипуляциям?"
  answer="Множество независимых операторов узлов (Deutsche Telekom, Swisscom и др.) получают данные из множества источников, агрегируют off-chain через медиану. Для BFT-устойчивости нужно n >= 3f+1 узлов. Манипуляция требует компрометации большинства узлов."
/>
