...
<KnowledgeCheck
  question="Что происходит со ставкой заимствования при резком росте utilization rate выше оптимального уровня (kinked curve)?"
  answer="Ставка резко возрастает (кинк/излом кривой). Например, при optimal utilization = 80%: ниже 80% ставка растет линейно и медленно. Выше 80% ставка взлетает в 10-20 раз, стимулируя возврат займов и привлечение новых депозитов."
/>
...
<SupplyBorrowFlowDiagram client:visible />
...
<ATokenGrowthDiagram client:visible />
...
<KnowledgeCheck
  question="В чем разница между LTV (Loan-to-Value) и Liquidation Threshold в Aave?"
  answer="LTV (например, 80%) ограничивает максимальный займ: при залоге $10,000 можно занять до $8,000. Liquidation Threshold (например, 82.5%) определяет порог ликвидации: если долг превысит 82.5% стоимости залога, позиция ликвидируется. Разрыв между ними -- буферная зона."
/>
