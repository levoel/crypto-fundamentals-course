...
<KnowledgeCheck
  question="Какие три обязательные проверки нужно выполнить перед использованием данных Chainlink latestRoundData()?"
  answer="1) price > 0 (защита от нулевой/отрицательной цены). 2) answeredInRound >= roundId (данные из текущего раунда). 3) block.timestamp - updatedAt < maxStaleness (данные не устарели). Без этих проверок контракт может использовать цену недельной давности."
/>
...
<KnowledgeCheck
  question="Почему TWAP-оракул устойчивее к flash loan манипуляциям, чем spot price?"
  answer="TWAP усредняет цену за N блоков. Flash loan действует в рамках 1 блока. Для манипуляции TWAP нужно удерживать искаженную цену N блоков подряд, что стоит пропорционально глубине пула * время -- экономически нецелесообразно."
/>
