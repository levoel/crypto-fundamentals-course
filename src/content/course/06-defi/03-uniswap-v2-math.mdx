...
<KnowledgeCheck
  question="Зачем Uniswap V2 использует множитель 997/1000 вместо десятичной дроби 0.997?"
  answer="Solidity не поддерживает числа с плавающей точкой. Целочисленная арифметика: amountIn * 997 / (reserveIn * 1000 + amountIn * 997) -- это эквивалент комиссии 0.3% без дробных чисел и погрешностей округления."
/>
...
<LPTokenMintingDiagram client:visible />
...
<KnowledgeCheck
  question="Зачем первые 1000 LP-токенов навсегда блокируются на address(0) при создании пула?"
  answer="MINIMUM_LIQUIDITY = 1000 предотвращает атаку 'inflation': без блокировки первый LP мог бы создать пул с 1 wei, затем отправить большую сумму напрямую в контракт, сделав каждый LP-токен сверхдорогим и блокируя последующих LP из-за округления."
/>
...
<V2vsV3EfficiencyDiagram client:visible />
