...
<KnowledgeCheck
  question="Какие 4 макроса составляют основу Anchor-программы и что генерирует каждый?"
  answer="#[program] -- entrypoint и dispatch по дискриминатору. #[derive(Accounts)] -- валидация constraints (owner, signer, PDA, has_one). #[account] -- Borsh сериализация и 8-байтный discriminator. #[error_code] -- типизированные ошибки с кодами."
/>
...
<AnchorChecksTableDiagram client:visible />
...
<KnowledgeCheck
  question="Как 8-байтный discriminator Anchor защищает от подмены типа аккаунта?"
  answer="При init Anchor записывает SHA-256('account:TypeName')[:8] в первые 8 байт data. При каждом чтении -- сравнивает. Если атакующий передаст аккаунт типа Profile вместо Counter, discriminator не совпадет и транзакция будет отклонена до вызова handler."
/>
