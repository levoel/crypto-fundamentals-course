...
<AnchorTestArchitectureDiagram client:visible />
...
<KnowledgeCheck
  question="Как MethodsBuilder API в Anchor строит и отправляет транзакцию?"
  answer="Цепочка вызовов: program.methods (выбор программы) -> .initialize() (инструкция из #[program]) -> .accounts({...}) (аккаунты из #[derive(Accounts)]) -> .signers([]) (дополнительные подписанты) -> .rpc() (отправка транзакции). Для чтения: program.account.counter.fetch(pda)."
/>
...
<KnowledgeCheck
  question="Почему негативный тест с wrong authority проваливает транзакцию в Anchor-программе?"
  answer="PDA вычисляется из seeds [b'counter', authority.key()]. Если authority != оригинальный, seeds дают другой PDA (ConstraintSeeds ошибка). Кроме того, has_one = authority проверяет counter.authority == authority.key() -- несовпадение вызывает CourseError::Unauthorized."
/>
...
# Ожидаемый результат:
#   course-counter
#     [OK] initializes the counter
#     [OK] increments the counter
#     [OK] increments again
#     [OK] fails with wrong authority
#   4 passing
```
